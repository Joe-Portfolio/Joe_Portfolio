...
const Header: React.FC<HeaderProps> = ({ lang, setLang, t, initialLang }) => {
  ...
  const safePlay = async () => {
    if (bgMusicRef.current && isMusicOn) {
      try {
        ...
        console.warn("BGM Playback interrupted or blocked:", error);
      }
    }
  };
  ...
  const toggleMusic = async () => {
    // TTS 중에는 작동하지 않도록 방어 로직 추가 (버튼 비활성화와 별개로 안전장치)
    if (!bgMusicRef.current || isSpeaking) return;
  ...
  const handleTts = async () => {
    ...
    try {
      ...
      if (base64Audio) {
        ...
        source.onended = () => {
          setIsSpeaking(false);
          // TTS 재생이 끝나면 BGM이 설정상 On일 경우에만 다시 재생
          if (isMusicOn) safePlay();
        };
        ...
        <div className="flex items-center space-x-4">
          <div className="hidden sm:flex items-center space-x-1.5 bg-slate-900/60 backdrop-blur-xl border border-white/5 rounded-full p-1 shadow-inner ring-1 ring-white/10">
            ...
            <button 
              ...
              title={isSpeaking ? "Narration in progress..." : "BGM Toggle"}
            >
              {isMusicOn ? (
                <svg className={`w-4 h-4 ${!isSpeaking && 'animate-pulse'}`} fill="currentColor" viewBox="0 0 24 24"><path d="M12 3v10.55c-.59-.34-1.27-.55-2-.55-2.21 0-4 1.79-4 4s1.79 4 4 4 4-1.79 4-4V7h4V3h-4z"/></svg>
              ) : (
