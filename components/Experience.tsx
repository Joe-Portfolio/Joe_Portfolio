  ...
  return (
    <div className="space-y-3">
      {displayLines.map((line, i) => (
        <div key={i} className="flex items-start text-slate-800 text-sm font-bold animate-fadeIn">
    ...
    <section id="experience" className="py-32 px-6 relative z-10">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 transition-all duration-700">
          {experiences.slice(0, isExpanded ? experiences.length : initialCount).map((exp, idx) => (
            <div 
              key={idx} 
              className="group relative p-10 rounded-[2.5rem] bg-slate-900/40 border border-slate-800/50 hover:border-blue-500/50 transition-all duration-500 backdrop-blur-xl animate-fadeIn cursor-help overflow-hidden flex flex-col min-h-[220px]"
            ...
            >
              ...
              <p className="text-sm text-slate-400 leading-relaxed line-clamp-2 mt-auto relative z-10">
                {exp.highlights[0]}
              </p>
        ...
        {/* Floating Post-it with Typing Animation */}
        {hoveredIdx !== null && (
          <div 
